# Libre MCP Bridge para ArcGIS Pro
...
| Pieza | Qué es |
|---|---|
| `LibreMcpAddin.esriAddinX` (`package/`) | Add-In compilado: 68 comandos + ventana de control manual |
...
> Si tu `Documentos` está redirigido a OneDrive, Pro usa esa ruta real automáticamente con el doble clic.
...
1. Abrí tu proyecto en Pro. Nada escucha solo: pestaña **Libre MCP** → botón **Puente MCP** → **Iniciar** (muestra `ACTIVO 127.0.0.1:5876`).
...
| `open_map` falla | Limitación real de Pro 3.5 (el API es 3.6+); abrí el mapa a mano |

## Notas técnicas

- Límites honestos: sin portal en línea (3 comandos responden error limpio), `open_map` requiere Pro 3.6+.
