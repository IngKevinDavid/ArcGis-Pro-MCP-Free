    ...
    public static class GeodatabaseCommands
    {
            ...
            string description)
        {
            ...
            if (result.IsFailed)
            {
                var messages = result.Messages != null
                    ? result.Messages.Select(message => "[" + message.Type + "] " + message.Text)
            ...
            return QueuedTask.Run<object>(() =>
            {
                ...
                var layer = MapView.Active != null && MapView.Active.Map != null
                    ? MapView.Active.Map.GetLayersAsFlattenedList()
                        ...
                        .FirstOrDefault(candidate =>
                            ...
                            || (candidate.GetPath() != null && candidate.GetPath().LocalPath.Equals(datasetPath, StringComparison.OrdinalIgnoreCase)))
                    : null;
                ...
                using (var definition = featureClass.GetDefinition())
                {
                    ...
                    return new
                    {
                        name = layer.Name,
                        type = "FeatureLayer",
                        path = layer.GetPath() != null ? layer.GetPath().LocalPath : "",
        ...
        private static object TryDescribeFileGeodatabaseDataset(string datasetPath)
        {
            ...
            string datasetName = datasetPath.Length == geodatabasePath.Length
                ? string.Empty
                : datasetPath.Substring(geodatabasePath.Length + 1).Replace(Path.DirectorySeparatorChar, '\\');
        ...
        private static string FindFileGeodatabasePath(string datasetPath)
        {
            ...
            var geodatabasePath = fullPath.Substring(0, index + marker.Length);
