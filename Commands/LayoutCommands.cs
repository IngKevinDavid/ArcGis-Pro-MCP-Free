...
namespace ArcGisProMcpFree.Commands
{
    public static class LayoutCommands
    {
            ...
            bool includeMapSeries)
        {
            return QueuedTask.Run<object>(() =>
            {
                ...
                foreach (var layoutItem in layoutItems)
                {
                    ...
                    catch (Exception ex) when (ex is InvalidOperationException || ex is IOException || ex is UnauthorizedAccessException || ex is ArgumentException)
                    {
                        failed++;
                        results.Add(new
                        {
        ...
        private static long GetFeatureCount(BasicFeatureLayer layer)
        {
            using (var table = layer.GetTable())
            {
                return table != null ? table.GetCount() : 0;
            }
        }
        ...
        private static MapSurroundInfo CreateMapSurroundInfo(MapSurroundType type)
        {
            switch (type)
            {
                case MapSurroundType.Legend: return new LegendInfo();
                case MapSurroundType.NorthArrow: return new NorthArrowInfo();
                case MapSurroundType.ScaleBar: return new ScaleBarInfo();
                default: throw new ArgumentException("Unsupported map surround type: " + type);
            }
        }
            ...
            int height)
        {
            ...
            {
                case "JPEG":
                case "JPG": exportFormat = new JPEGFormat(); break;
                case "PDF": exportFormat = new PDFFormat(); break;
                default: exportFormat = new PNGFormat(); break;
            }
        ...
        private static ExportFormat CreateExportFormat(string outputPath, string formatType, int dpi)
        {
            ...
            switch (formatType.ToUpperInvariant())
            {
                case "JPEG":
                case "JPG": exportFormat = new JPEGFormat(); break;
                case "PNG": exportFormat = new PNGFormat(); break;
                case "TIFF":
                case "TIF": exportFormat = new TIFFFormat(); break;
                default: exportFormat = new PDFFormat(); break;
            }
